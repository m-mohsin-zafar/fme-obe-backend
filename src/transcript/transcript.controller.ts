...
export class TranscriptController {
    ...
    @Get(':id')
    async getTranscriptById(@Param('id') id: String, @Res({passthrough: true}) res: Response) {
        try {
    ...
    @Get('pdf/:id')
    async getTranscriptPDFById(@Param('id') id: String, @Res({passthrough: true}) res: Response) {
        try {
            const transcript = await this.transcript.getTranscriptByRegNo(Number(id));
    ...
    @Get('pdf/batch/:batch')
    async getTranscriptPDFsByBatch(@Param('batch') batch: String, @Res({passthrough: true}) res: Response) {
        try {
            const zipFilePath = await this.transcript.getTranscriptsByBatch(Number(batch));
